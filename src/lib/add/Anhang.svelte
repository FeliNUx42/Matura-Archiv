<script>
  ...
  $: errors = $err_full
    .filter(e => e.path[0] == "anhang" && e.path[1] == i)
    .map(e => e.message)
</script>
...
<div transition:slide={{duration:250}}>
  <div class="rounded-md border border-gray-300 flex divide-x divide-gray-300 dark:border-gray-600 dark:divide-gray-600 {errors.length ? "mb-1" : ""}">
    ...
    <div class="input-group relative transition duration-250 hover:text-gray-800">
      <input name="anhang-name-{i}" placeholder=" " class="px-4 py-2 w-full text-gray-800 outline-none bg-transparent dark:text-gray-200"/>
      <label class="block pl-4 pt-2 text-gray-400 m-0 absolute top-px left-px duration-250 transition-all ease-in-out rounded-md bg-white dark:bg-dark-900 dark:text-gray-500" for="anhang-name-{i}">
        Name
      </label>
    </div>
    <label class="text-gray-400 px-4 py-2 block bg-gray-100 dark:bg-dark-800" for="anhang-datei-{i}">
      Datei
    </label>
    <input id="anhang-datei-{i}" name="anhang-{i}" type="file" class="grow text-gray-500 px-4 py-2 hover:text-gray-800 dark:hover:text-gray-400"/>
    <button 
      type="button"
